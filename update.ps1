...
`$Global:PSProfileRepoOwner = '$escapedOwner'
`$Global:PSProfileRepoName = '$escapedName'
`$Global:PSProfileRepoBranch = '$escapedBranch'
...
$existingRepoConfigPaths = $repoConfigPaths | Where-Object { Test-Path $_ }
$existingConfigPath = $existingRepoConfigPaths | Select-Object -First 1
if ($existingConfigPath) {
    try {
        . $existingConfigPath
    } catch {
        Write-Warning "Failed to load repo config from '$existingConfigPath': $($_.Exception.Message)"
    }
}
