...
if ($configNeeded) {
    # Escape single quotes to keep generated PowerShell strings safe
    $EscapedRepoOwner       = ('' + $RepoOwner)       -replace "'", "''"
    $EscapedRepoName        = ('' + $RepoName)        -replace "'", "''"
    $EscapedRepoBranch      = ('' + $Branch)          -replace "'", "''"
    $EscapedRepoBase        = ('' + $RepoBase)        -replace "'", "''"
    $EscapedGitPromptStyle  = ('' + $GitPromptStyle)  -replace "'", "''"

    $RepoConfigContent = @"
# Auto-generated by ps-profile installer
`$Global:PSProfileRepoOwner = '$EscapedRepoOwner'
`$Global:PSProfileRepoName = '$EscapedRepoName'
`$Global:PSProfileRepoBranch = '$EscapedRepoBranch'
`$Global:PSProfileRepoBase = '$EscapedRepoBase'
`$Global:PSProfileGitPromptStyle = '$EscapedGitPromptStyle'
"@
